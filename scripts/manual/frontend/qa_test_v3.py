"""
QA Test Script for Infinite Canvas - Version 3
        ...
        try:
            ...
            if new_node_btn.count() > 0:
                ...
                if image_option.count() > 0:
                    ...
                    print(f"  Image option not found, trying other selectors...")
                    # Try to find any button in the popup
                    popup_buttons = page.locator(
                        '[class*="popup"] button, [class*="menu"] button, [class*="dropdown"] button'
                    )
                    print(f"  Found {popup_buttons.count()} popup buttons")
            ...
            if image_node.count() > 0:
                ...
                # Check for side ports - they should be visible when node is selected
                # The ports are in MediaSidePorts component with handles
                left_port = page.locator('[data-testid="image-node-input-port"]')
                right_port = page.locator('[data-testid="image-node-output-port"]')

                # Also check for the + buttons on left and right
                left_plus = page.locator(
                    '.react-flow__node:has([data-preview-frame="image"]) >> text=+'
                ).first
                right_plus = page.locator(
                    '.react-flow__node:has([data-preview-frame="image"]) >> text=+'
                ).last
                ...
                # Also check if the node has overflow-visible (not clipped)
                ...
                print(f"  No image node found. Total nodes: {all_nodes.count()}")
                for i in range(all_nodes.count()):
                    node_html = all_nodes.nth(i).inner_html()[:100]
                    print(f"    Node {i}: {node_html}")
            ...
            # Look for settings button (aria-label='设置')
            settings_btn = page.locator('button[aria-label="设置"]')

            if settings_btn.count() > 0:
                print(f"  Found settings button, clicking...")
                settings_btn.first.click()
                time.sleep(1)

                page.screenshot(
                    path="D:/个人项目/pp/infinite-canvas/frontend/tmp/qa_settings_click.png",
                    full_page=True,
                )

                # Look for login/register option in the menu
                login_option = page.locator(
                    'button:has-text("登录"), a:has-text("登录"), text=登录'
                )
                if login_option.count() > 0:
                    print(f"  Found login option")
                    login_option.first.click()
                    time.sleep(1)

            page.screenshot(
                path="D:/个人项目/pp/infinite-canvas/frontend/tmp/qa_login_page.png",
                full_page=True,
            )

            # Check if we're on a login page/modal
            username_input = page.locator('input[name="username"], input[type="text"]')
            ...
            if username_input.count() > 0 and password_input.count() > 0:
                ...
                register_tab = page.locator(
                    'text=注册, a:has-text("注册"), button:has-text("注册")'
                )
                ...
                if submit_btn.count() > 0:
                    ...
                    # Now look for user menu or profile button
                    # After login, there should be a user avatar or profile button
                    user_elements = page.locator(
                        '[class*="avatar"], [class*="user"], [class*="profile"]'
                    )
                    print(f"  Found {user_elements.count()} user-related elements")

                    # Try clicking on user avatar or profile
                    if user_elements.count() > 0:
                        user_elements.first.click()
                        time.sleep(1)

                        page.screenshot(
                            path="D:/个人项目/pp/infinite-canvas/frontend/tmp/qa_user_menu.png",
                            full_page=True,
                        )
                        ...
                        results["personal_center_settings"]["status"] = "FAIL"
                        results["personal_center_settings"]["details"] = (
                            "User elements not found after login"
                        )
                        print("  [FAIL] User elements not found")
                else:
                    results["personal_center_settings"]["status"] = "FAIL"
                    results["personal_center_settings"]["details"] = (
