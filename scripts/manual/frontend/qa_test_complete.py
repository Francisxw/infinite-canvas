"""
QA Test Script for Infinite Canvas - Complete Version
        ...
        try:
            ...
            if buttons.count() >= 7:
                ...
                email_input = page.locator(
                    'input[type="text"], input[name="email"], input[placeholder*="name@example.com"]'
                )
                ...
                if email_input.count() > 0 and password_input.count() > 0:
                    ...
                    if submit_btn.count() > 0:
                        ...
                        if has_text and has_image and has_video:
                            ...
                            # Check if we need to look for the settings in a different way
                            # Maybe after login, we need to click on settings again
                            settings_link = page.locator(
                                'text=设置, text=Settings, a:has-text("设置")'
                            )
                            if settings_link.count() > 0:
                                ...
                                page.screenshot(
                                    path="D:/个人项目/pp/infinite-canvas/frontend/tmp/qa_settings_after_login.png",
                                    full_page=True,
                                )

                                # Check again for model selectors
                                text_model = page.locator(
                                    'text=文本模型, text=Text Model, label:has-text("文本")'
                                )
                                image_model = page.locator(
                    ...
                    results["personal_center_settings"]["status"] = "FAIL"
                    results["personal_center_settings"]["details"] = (
                        "Email/password inputs not found"
                    )
                    print("  [FAIL] Email/password inputs not found")
            else:
                results["personal_center_settings"]["status"] = "FAIL"
                results["personal_center_settings"]["details"] = (
