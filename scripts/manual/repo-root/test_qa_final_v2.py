...
def test_blank_canvas(page):
    ...
    # Navigate to the app via localhost (not 127.0.0.1 to avoid CORS issues)
...
def test_personal_center_settings(page):
    ...
    # Now navigate to the app and login via localhost (not 127.0.0.1 to avoid CORS issues)
    page.goto("http://localhost:5180")
    page.wait_for_load_state("networkidle")
    time.sleep(2)
