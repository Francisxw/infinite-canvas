...
import type { VideoNodeShape } from './types'
...
export class VideoNodeShapeUtil extends BaseBoxShapeUtil<VideoNodeShape> {
  ...
  override getDefaultProps(): VideoNodeShape['props'] {
    return {
      w: 620,
      h: 300,
      dataUrl: null,
  ...
  override getHandles(shape: VideoNodeShape) {
    return getBoxPortHandles(shape.props.w, shape.props.h)
  }

  override getGeometry(shape: VideoNodeShape) {
    return new Rectangle2d({ width: shape.props.w, height: shape.props.h, isFilled: true })
  }

  override onResize(shape: VideoNodeShape, info: TLResizeInfo<VideoNodeShape>) {
  ...
  component(shape: VideoNodeShape) {
    const onPickFile = async () => {
      ...
      input.onchange = async () => {
        ...
        const partial: TLShapePartial<VideoNodeShape> = {
          id: shape.id,
          type: shape.type,
          props: {
    ...
    return (
      <HTMLContainer style={{ pointerEvents: 'all' }}>
        <NodeFrame title="Video" icon={<Video className="h-4 w-4" />}>
          ...
          <RoundedCard>
            <div className="flex h-full items-center justify-center rounded-2xl border border-white/20 bg-[#252525]">
              {shape.props.dataUrl ? (
                <video src={shape.props.dataUrl} controls className="h-full w-full rounded-2xl object-cover" />
              ) : (
  ...
  indicator(_shape: VideoNodeShape) {
    return null
  }
}
